...
exports.mergeBkashCredentials = mergeBkashCredentials;
exports.isBkashConfigComplete = isBkashConfigComplete;
exports.isBkashMerchantCleanupActive = isBkashMerchantCleanupActive;
exports.listPublicPaymentMethods = listPublicPaymentMethods;
exports.assertCheckoutGatewayAllowed = assertCheckoutGatewayAllowed;
exports.isMfsReferenceRow = isMfsReferenceRow;
exports.isBankProofRow = isBankProofRow;
exports.findOptionForGateway = findOptionForGateway;
exports.isManualVerificationGateway = isManualVerificationGateway;
exports.isBankProofGateway = isBankProofGateway;
exports.isMfsReferenceGateway = isMfsReferenceGateway;
exports.listAllForAdmin = listAllForAdmin;
exports.createOption = createOption;
exports.updateOption = updateOption;
exports.removeOption = removeOption;
const config_1 = require("../config");
const errorHandler_1 = require("../middlewares/errorHandler");
const repo = __importStar(require("../repositories/paymentOptionRepository"));
...
function mergeBkashCredentials(row) {
    const m = row?.merchant_credentials ?? {};
    const base = {
        baseUrl: pickStr(m.base_url, config_1.env.bkash.baseUrl).replace(/\/$/, ''),
        username: pickStr(m.username, config_1.env.bkash.username),
        password: pickStr(m.password, config_1.env.bkash.password),
        appKey: pickStr(m.app_key, config_1.env.bkash.appKey),
        appSecret: pickStr(m.app_secret, config_1.env.bkash.appSecret),
        agreementId: pickStr(m.agreement_id, config_1.env.bkash.agreementId),
        callbackBaseUrl: pickStr(m.callback_base_url, config_1.env.bkash.callbackBaseUrl).replace(/\/$/, ''),
    };
    if (!base.username ||
        !base.password ||
        !base.appKey ||
...
function isBkashConfigComplete(cfg) {
    return cfg != null;
}
/** Whether bKash stale-order cleanup should run (enabled merchant option or legacy env). */
async function isBkashMerchantCleanupActive() {
    const rows = await repo.findEnabledPublic();
    const bk = rows.find((r) => r.kind === 'merchant' && r.gateway_key === 'bkash');
    if (bk && mergeBkashCredentials(bk))
        return true;
    return config_1.env.bkash.enabled && mergeBkashCredentials(null) != null;
}
function bankDetailsFromRow(row) {
...
function applyEnvBankDetailOverrides(gatewayKey, details) {
...
async function listPublicPaymentMethods() {
    const rows = await repo.findEnabledPublic();
    const out = [];
...
async function assertCheckoutGatewayAllowed(gatewayKey) {
    const row = await repo.findEnabledByGatewayKey(gatewayKey);
    if (!row) {
        throw new errorHandler_1.AppError(400, 'This payment method is not available');
    }
    if (row.kind === 'merchant' && row.gateway_key === 'bkash') {
        if (!mergeBkashCredentials(row)) {
            throw new errorHandler_1.AppError(503, 'bKash checkout is not fully configured.');
        }
    }
    return row;
}
function isMfsReferenceRow(row) {
    return row.kind === 'manual' && row.manual_flow === 'mfs_reference';
}
function isBankProofRow(row) {
...
async function findOptionForGateway(gatewayKey) {
    return repo.findByGatewayKey(gatewayKey);
}
async function isManualVerificationGateway(gatewayKey) {
...
async function isBankProofGateway(gatewayKey) {
    const row = await repo.findByGatewayKey(gatewayKey);
    if (!row)
        return gatewayKey === 'manual';
    return isBankProofRow(row);
}
async function isMfsReferenceGateway(gatewayKey) {
    const row = await repo.findByGatewayKey(gatewayKey);
    if (!row) {
...
function maskMerchantCredentials(row) {
...
function validateGatewayKey(key) {
    if (!/^[a-z][a-z0-9_]{1,62}$/.test(key)) {
        throw new errorHandler_1.AppError(400, 'gateway_key must start with a letter and contain only lowercase letters, numbers, and underscores (2–63 chars).');
    }
}
async function createOption(body) {
    ...
    if (body.kind === 'merchant') {
        if (body.gateway_key !== 'bkash') {
            throw new errorHandler_1.AppError(400, 'Merchant integration currently supports gateway_key "bkash" only.');
        }
        const n = await repo.countByKind('merchant');
        if (n > 0)
            throw new errorHandler_1.AppError(400, 'A bKash merchant option already exists. Edit the existing one.');
        if (body.manual_flow != null)
            throw new errorHandler_1.AppError(400, 'manual_flow must be null for merchant options.');
    }
    else {
        if (!body.manual_flow)
            throw new errorHandler_1.AppError(400, 'manual_flow is required for manual options (mfs_reference or bank_proof).');
        if (body.manual_flow === 'bank_proof' && !body.bank_details) {
            throw new errorHandler_1.AppError(400, 'bank_details are required for bank transfer (bank_proof) methods.');
        }
        if (body.manual_flow === 'mfs_reference' && !body.bank_details) {
            throw new errorHandler_1.AppError(400, 'bank_details (pay-to instructions) are required for wallet manual methods.');
        }
    }
    const dup = await repo.countByGatewayKey(body.gateway_key);
    if (dup > 0)
        throw new errorHandler_1.AppError(409, 'This gateway_key is already in use.');
    ...
    if (!row)
        throw new errorHandler_1.AppError(500, 'Failed to load created payment option');
    const pub = rowToPublic(row);
    return {
        ...pub,
        payment_option_id: row.id,
...
function mergeCredentialPatch(existing, patch) {
    if (!patch)
    ...
    for (const [k, v] of Object.entries(patch)) {
        if (v === undefined)
            continue;
        if (secretKeys.includes(k) && (v === '' || v === '__unchanged__'))
            continue;
        out[k] = v;
    }
...
async function updateOption(id, patch) {
    const row = await repo.findById(id);
    if (!row)
        throw new errorHandler_1.AppError(404, 'Payment option not found');
    let mergedCred;
    ...
    if (!updated)
        throw new errorHandler_1.AppError(500, 'Failed to load payment option');
...
//# sourceMappingURL=paymentOptionService.js.map