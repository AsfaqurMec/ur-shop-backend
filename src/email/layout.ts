...
import { getStoreSettingsSnapshot } from '../services/storeSettingsService';
...
function isEmailSafeImageSrc(value: string): boolean {
  const v = value.trim();
  if (!v) return false;
...
export function mobileFieldLabel(text: string): string {
  return `<span class="m-label">${escapeHtml(text)}</span>`;
}

function emailHeadStyles(title: string): string {
...
export function getStoreName(): string {
  const settings = getStoreSettingsSnapshot();
  return settings.siteTitle || settings.storeName || env.store.name;
}

export function wrapHtml(title: string, bodyHtml: string): string {
  const settings = getStoreSettingsSnapshot();
  const brandName = settings.siteTitle || settings.storeName || env.store.name;
  const name = escapeHtml(brandName);
  const tagline = escapeHtml(settings.emailHeaderSlogan || env.store.tagline);
  const subTitle = escapeHtml(settings.emailHeaderSubtitle || '');
  const headerLogo = settings.emailHeaderLogo || settings.siteLogo;
  const shopUrl = env.frontendUrl ? escapeAttr(env.frontendUrl) : '';
  const shopLink = env.frontendUrl
    ? `<a href="${shopUrl}" style="color:#fff;text-decoration:none;font-weight:600">${name}</a>`
    : `<span style="font-weight:600">${name}</span>`;
  const supportEmail = settings.emailFooterSupportEmail || settings.contactEmail || env.store.supportEmail || '';
          ...
          <tr>
            <td class="email-footer" style="padding:20px 28px 28px;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:12px;color:${TEXT_MUTED};border-top:1px solid #e7e5e4;background:#fafaf9;text-align:center;word-break:break-word;overflow-wrap:break-word">
              ...
              <p style="margin:0 0 10px">
                ${env.frontendUrl ? `<a href="${shopUrl}" style="color:${ACCENT};text-decoration:none">Visit shop</a>` : ''}
                ${env.frontendUrl && support ? ' · ' : ''}
