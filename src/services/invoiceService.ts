...
function getStoreBrandName(settings: { siteTitle?: string; storeName?: string }): string {
  const isBoilerplate = (s: string) =>
    /^digital product(s)? selling$/i.test(s) ||
...
async function loadCompanyLogo(urlOrPath: string | null | undefined): Promise<Buffer | null> {
  ...
  if (trimmed.startsWith('/') || !/^https?:\/\//i.test(trimmed)) {
    ...
    const candidates = [
      ...
      path.join(process.cwd(), '..', cleanPath),
      path.join(process.cwd(), '..', 'UR-Shop--Frontend', 'public', cleanPath),
    ];
    for (const c of candidates) {
      if (fs.existsSync(c)) {
        try {
          const buf = fs.readFileSync(c);
          if (buf.length > 0 && buf.length <= 2_000_000) return buf;
        } catch {}
      }
    }
  }
...
export async function createInvoicePdf(
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.text(
      `Thank you for shopping with ${brand}. Generated on ${new Date().toLocaleDateString('en-GB')}.`,
      MARGIN_X,
      PAGE_HEIGHT - 28,
      { lineBreak: false }
    );
